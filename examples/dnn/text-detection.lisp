...
(defparameter *model-path* "frozen_east_text_detection.pb")
...
(defun detect-text (frame)
  (with-resource (blob (blob-from-images (list frame)
    ...
    (destructuring-bind (scores geometry)
        (forward *model* :names *model-output-layers* :input blob)
      (let* ((sz (shape scores))
             (sh (elt sz 2))
             (sw (elt sz 3))
...
(defun process-frame (img)
  (let* ((regions (detect-text img))
         (freq (/ (tick-frequency) 1000.0))
         (time (/ (performance-profile *model*) freq)))
    (draw-regions img regions)
    (draw-perf-info img (format nil "Inference time: ~,2f ms" time))
    img))

(defun text-detection (input)
  (load-model)
  ...
  (let ((options (opts:get-opts (or args (opts:argv)))))
    (if (getf options :help)
        ...
        (macrolet ((setparameter (param key)
          ...
          (setparameter *confidence-threshold* :confidence-threshold)
          (setparameter *nms-threshold* :nms-threshold)
          (text-detection (getf options :input))))))
